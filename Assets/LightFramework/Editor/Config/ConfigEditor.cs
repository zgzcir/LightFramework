...
using static LightFramework.Editor.Resource.PathDefineEditor;
using static LightFramework.Editor.Resource.ProfileAccessor;
using static LightFramework.Editor.Config.ReflectionExtensions;

namespace LightFramework.Editor.Config
{
    public class ConfigEditor
    {
        ...
        private static void XmlGenExcelProcess(string name)
        {
            ...
            string xlslPath = GetFrameProfile().ExcelFullPath(excelName);
        ...
        private static void ExcelGenXmlProcess(string name)
        {
            ...
            string excelName = null;
            Dictionary<string, Sheet> sheetDic = ReadSheet(name, ref excelName, ref xmlName, ref className);
            string excelPath = GetFrameProfile().ExcelFullPath(excelName);
            Dictionary<string, SheetData> sheetDataDic = new Dictionary<string, SheetData>();
                        ...
                        foreach (var worksheet in worksheets)
                        {
                            ...
                            for (int r = 1; r < sheetRowCount; r++)
                            {
                                ...
                                for (; c < sheetColCount; c++)
                                {
                                    ...
                                    string key = GetNameFromCol(sheet.Variables,
                                            worksheet.Cells[1, c + 1].Value.ToString())
                                        .Trim();
        ...
        {
            string regPath = GetFrameProfile().RegFullPath(name);
        ...
        public static void TestObjectReflection()
        {
            ...
            for (int i = 0; i < listCount; i++)
            {
                var item = list.GetType().InvokeMember("get_Item", BindingFlags.Default | BindingFlags.InvokeMethod,
                    null,
                    list, new object[] {i});
                var content =
